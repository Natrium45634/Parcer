...
ENTER_TEMPLATES = (
    "%(who)s входит в место по имени %(place)s — первым за %(years)s.",
...
FAIL_TEMPLATES = (
    "%(who)s входит в место по имени %(place)s и не выходит. Искать не идут.",
    "Из тех, кто спустился в место по имени %(place)s, наверх не "
    "поднимается никто.",
    "Место по имени %(place)s принимает ещё одного: %(who)s остаётся там "
    "навсегда.",
...
SETTLED_TEMPLATES = (
    "В месте по имени %(place)s поселяется то, чего там прежде не было. "
    "Окрестные деревни узнают об этом первыми.",
    "Пустое место недолго остаётся пустым: там, где стоит %(place)s, "
    "кто-то живёт.",
    "Охотники приносят весть: в месте по имени %(place)s горит огонь, "
    "а жечь его некому.",
)
...
def site_settled(rng, site, guard: str) -> tuple:
    ...
    if guard and guard != sites_mod.NOBODY:
        text = "%s Что стережёт это место: %s." % (text, guard)
