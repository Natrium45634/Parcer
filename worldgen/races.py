...
# Титул по форме страны. Раса задаёт свой лад («король-под-горой»,
# «владычица»), но если страна зовётся империей или герцогством, титул
# должен отвечать её званию: империей правит не король, а император.
# Ключ ищется как подстрока в названии формы, поэтому «Подгорное
# Королевство» подходит под «Королевство».
FORM_TITLES = (
    ("Империя", ("император", "императрица")),
    ("Герцогство", ("герцог", "герцогиня")),
    ("Княжество", ("князь", "княгиня")),
    ("Ханство", ("хан", "ханша")),
    ("Вождество", ("вождь", "вождица")),
    ("Союз", ("старший союза", "старшая союза")),
)

# У выборных союзов титул звучит по-своему, но раса всё же слышна:
# у дворфов «старший клана», у птицелюдов «старший гнёзд».
UNION_TITLES = {
    "Клановый Союз": ("старший кланов", "старшая кланов"),
    "Вольный Союз": ("первый среди равных", "первая среди равных"),
    "Союз Стай": ("вожак стай", "вожачка стай"),
    "Союз Гнёзд": ("старший гнёзд", "старшая гнёзд"),
    "Песчаный Союз": ("шейх", "шейха"),
    "Горный Союз": ("старший гор", "старшая гор"),
    # Торговой республикой правит не государь, а выборный глава гильдии.
    "Торговая Республика": ("дож", "дожесса"),
    "Вольный Город": ("бургомистр", "бургомистрша"),
}


def title_for_form(form: str, sex: str, fallback) -> str:
    """Титул правителя по форме страны; если форма ничего не диктует — расовый."""
    if form:
        exact = UNION_TITLES.get(form)
        if exact is not None:
            return exact[1] if sex == "f" else exact[0]
        for key, pair in FORM_TITLES:
            if key in form:
                return pair[1] if sex == "f" else pair[0]
    if not fallback:
        return "вождица" if sex == "f" else "вождь"
    return fallback[1] if (sex == "f" and len(fallback) > 1) else fallback[0]
...
ELECTIVE_FORMS = (
    ...
    "Торговая Республика", "Вольный Город",
)


# Общие множители, которыми настройки генератора правят сразу весь мир:
# скорость, с какой прибывает народу, и то, насколько шатко стоят престолы.
# По умолчанию — единица: движок работает так, как задуман.
GROWTH_SCALE = 1.0
COUP_SCALE = 1.0
...
RACES = (
    ...
    Race(
        ...
        # «матрон» — не слово: мужской формы у «матроны» нет. Мужчина
        # во главе дома у тёмных эльфов зовётся иначе.
        chief_titles=_t("старший дома", "матрона"),
        ...
        noble_titles=(("старший дома", "матрона"), ("наставник", "наставница")),
