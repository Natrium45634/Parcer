...
# Сколько гексов в «обычной» земле: столько выходит на малой карте
# 160×96, под которую и подобраны все числа движка. Земля крупнее кормит
# больше народу, мельче — меньше.
REFERENCE_HEXES = 210.0
...
def _classify(regions, wmap, ctx, rng) -> None:
    ...
    # Земля кормит по своей величине: на большой карте земель немногим
    # больше, зато каждая впятеро шире, и держать она должна впятеро
    # больше народу. Без этого мир на пятьсот гексов в ширину населён
    # так же, как на полтораста, а это и есть та несообразность, из-за
    # которой огромный мир выходил пустым.
    #
    # Зависимость не прямая: вдвое большая земля кормит не вдвое больше.
    # Дальние её углы приходится обживать отдельно, и раздел земли между
    # городами в неё уже заложен.
    used = set()
    for region in regions:
        ...
        worth = base * (0.55 + 0.9 * region.habitat)
        span = (max(1, len(region.hexes)) / REFERENCE_HEXES) ** 0.5
        region.capacity = round(worth * max(0.45, min(2.8, span)), 3)
