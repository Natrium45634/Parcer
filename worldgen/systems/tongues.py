...
def close(ctx, year: int) -> None:
    """Последний счёт языков, когда мир дописан.

    Десятилетний такт идёт раньше растворения народов и раньше великих
    бедствий: народ мог перенять чужую речь или уйти под воду уже после
    него. Тогда язык остаётся числиться живым, хотя говорить на нём
    стало некому, — и починить это некому, тактов больше нет. Здесь мир
    досчитывает последнее: кто кончился, тот кончился.
    """
    world = ctx.world
    rng = ctx.rng("tongues_close", year)
    world.refresh_folks()
    _fade_folks(ctx, year, rng)
    _count_speakers(world)
    _maybe_die(ctx, year, 0, rng)
    _state_tongues(world)
