...
TRIBE_END_TEMPLATES = (
    ...
    "%(what)s уходит за перевал, и обратно никто не возвращается.",
    "Два тяжёлых года подряд — и %(what)s больше нет: кто выжил, прибился "
    "к чужим кострам.",
    "%(what)s распадается на семьи, и семьи расходятся в разные стороны.",
    "Мор выкашивает %(what)s за одно лето. Стоянку обходят стороной.",
    "%(what)s снимается с места и больше нигде не встаёт лагерем.",
    "Соседи забирают у %(what)s угодья, потом стада, потом имя.",
    "После долгой зимы от %(what)s остаются шесть очагов, к весне — ни одного.",
    "%(what)s уходит в леса и перестаёт отвечать на зов сородичей.",
    "Ни вождя, ни старейшин: %(what)s кончается без единого решения.",
    "%(what)s гибнет в чужой усобице, в которую его позвали союзником.",
)
...
FOLK_END_TEMPLATES = (
    "%(what)s больше нет: последних его людей не стало, и некому назвать "
    "себя этим именем.",
    "Имя «%(name)s» перестаёт быть именем народа и становится именем "
    "в чужих песнях.",
    "%(what)s угасает не в один день: сперва перестают рождаться дети, "
    "потом некому становится хоронить стариков.",
    "Последние, кто звал себя так, расходятся по чужим очагам. "
    "%(what)s кончается тихо.",
    "О народе по имени %(name)s говорят теперь только в прошедшем времени.",
)

FOLK_END_TAILS = (
    "Земля, где он проснулся, зовётся его именем ещё долго.",
    "Соседи занимают его места не сразу: сперва боятся, потом забывают, "
    "чего боялись.",
    "Слова его подбирают чужие языки и носят, не помня, чьи они.",
    "Счёт годам его жизни сводят потом, задним числом, и каждый свод — свой.",
)


def folk_end(rng, folk, race, years: int) -> tuple:
    """Народ кончился — отдельное событие, которое мир помнит."""
    data = {"what": "Народ по имени %s" % folk.name, "name": folk.name}
    text = rng.choice(FOLK_END_TEMPLATES) % data
    text = "%s Он прожил %s. %s" % (text, years_text(max(1, years)),
                                    rng.choice(FOLK_END_TAILS))
    return ("Конец народа: %s" % folk.name), cap(text)
...
CAMP_END_TEMPLATES = (
    ...
    "%(what)s вырезают свои же — из-за добычи, которую не поделили.",
    "От %(what)s остаются частокол и кости; кто это сделал, не говорят.",
    "%(what)s снимается на промысел и не возвращается.",
    "Голод разгоняет %(what)s раньше, чем это успевают сделать соседи.",
    "%(what)s накрывают ночью и не оставляют никого.",
)
