...
Файл самодостаточный: внутри и движок, и окно, и значок, и пример карты.
Windows может предупредить, что программа не подписана, — «Подробнее» →
«Выполнить в любом случае».

Сборку делает GitHub Actions (`.github/workflows/build-windows.yml`) при
каждом пуше; готовый файл лежит в артефактах запуска. Новый выпуск в
Releases делается либо меткой версии (`git tag v1.0`), либо запуском
сборки вручную: **Actions → Хронист для Windows → Run workflow**, где
надо ответить «да» на вопрос о выпуске и назвать метку.
...
Программа открывается **мастером создания мира**:

1. **Мир** — сид и длительность. Этого хватит. Ниже — 68 шкал тонкой
   настройки, каждую можно зафиксировать замком, чтобы «Бросить кости»
   её не трогало.
2. **Карта** — случайная гексовая карта (её делает сама программа),
   свой файл `.world` или вовсе без карты. Случайную карту видно сразу,
   её можно перебрасывать и сохранять файлом.
3. **Создание** — сводка выбранного и одна кнопка.

После генерации окно переключается на летопись; кнопка «Новый мир»
возвращает к мастеру.
...
* **Сид.** Код из букв и цифр вида `KR7M-93XD`: две четвёрки через дефис.
  В нём нет знаков, которые путают на письме и на слух — ни буквы «O»
  рядом с нулём, ни «I» рядом с единицей, ни «B», «S» и «Z». Код можно
  набирать как удобно: строчными, без дефиса, с кириллическими буквами,
  похожими на латинские, — «kr7m93xd», «KR7M-93XD» и «КR7М 93ХD» дают
  один и тот же мир. Кнопка «Случайный» подбирает новый код, а вместо
  кода можно вписать любое слово: «Начало» и «Первый мир» — такие же
  полноправные сиды.
...
### Блок 5 — история на настоящей гексовой карте

Генератор строит историю не на выдуманной сетке земель, а на
**настоящей гексовой карте** — той, что делает встроенный Worldforge
(см. блок 23), или любом файле `.world` того же формата. В мастере это
шаг «Карта»: сделать свою, выбрать файл или обойтись без карты.
...
### Блок 11 — языки народов, посольства, унии, тайная политика и гильдии

**Языки.** У каждого народа свой язык, а у языка — свои **звуковые
законы**: правила, которые применяются ко всем его именам и всегда.
Там, где праязык говорит «Аргенталь», дочернее наречие скажет
«Алгендаль», а третье — «Оргендол». Слова остаются узнаваемо
родственными и при этом разными, как и положено языкам одной семьи, —
и потому города двух человеческих народов, разошедшихся тысячу лет
назад, больше не зовутся одинаковыми наборами букв.

* **Праязык** рождается вместе с расой; каждый отделившийся народ сперва
  говорит на нём же.
* **Расхождение.** Прожив врозь несколько веков, народ обзаводится
  собственным наречием: оно наследует законы родителя и добавляет
  один-два своих. Двадцать восемь законов — укание, оглушение,
  ламбдаизм, носовое, усечение и прочие.
* **Письменность** изобретают один раз (двенадцать видов — от рун до
  письма иглой по воску) и потом одалживают у соседей вместе с их
  словами; у каждой в справочнике сказано, чем она хороша и чем плоха.
* **Заимствования** приносит торговля вернее любого указа.
* **Смерть языка.** Язык умолкает вместе с народом — но иногда остаётся
  жить в храме как язык обряда, на котором уже никто не говорит дома.
* В справочнике «Языки» видны семьи, законы каждого наречия, сравнительный
  словник (одни и те же слова через законы каждого языка) и то, при каких
  дворах на нём пишут грамоты.

**Посольства.** Между отношением держав и подписанным договором встал
человек: посол с именем, свитой и наказом. Тринадцать наказов — от
поклона новому государю и разведения спорной межи до требования дани и
покорности; в дар везут то, чем держава богата. Двор слушает и решает,
считая приязнь, родство народов, нрав государя, дерзость просьбы,
богатство даров и умение самого посла. Ответов четыре: **приняли**
(и тогда заключается договор, кончается война, платится дань, выкупаются
пленные), **отказали**, **выставили** или **убили посла**.

**Обиды, у которых есть виновник.** Кровь посла, яд при дворе, пойманный
соглядатай, нарушенная клятва — всё это записывается за державой с годом
и припоминается при объявлении войны. Свежая обида весит больше любой
спорной межи и стирается за два с половиной века.

**Династические унии.** Когда престол пустеет, а законных наследников
нет, старую брачную грамоту достают из ларца: право на корону
оказывается у государя соседней державы. Державы не воюют за престол и
не сливаются — у них становится **один государь**. Уния держится, пока
обе короны достаются одному наследнику; разошлись наследники —
разошлись и державы. А если под одной короной выросло второе поколение,
младшая держава может и вовсе раствориться в старшей: без единого
выстрела, одной грамотой.

**Тайная политика.** Пока послы кланяются, по тем же дорогам едут люди
без грамот. Шесть тайных дел: **подкупить чужого воеводу** накануне
сражения, **выведать замыслы**, **свить заговор** среди недовольной
знати, **поднять на смуту** притесняемый народ, **выправить подложную
родословную** на чужую корону, **поднести яд** государю. Дело удаётся,
срывается втуне или раскрывается. Купленный воевода отрабатывает
серебро в ближайшей битве, и в летописи это видно; заговор шатает
престол и приводит к перевороту; подлог даёт притязание на чужую корону;
яд опустошает престол; а раскрытый соглядатай оставляет обиду, за
которую потом воюют.

**Гильдии и вольные города.** В богатом торговом городе заводится
гильдия — купеческая, мореходная, ремесленная или банкирская — со своей
кассой, складами и правом судить торговый спор без государя. Касса
растёт с каждого действующего пути и худеет от всякой войны, а деньги
идут в политику: гильдия покупает вольности при чужих дворах, нанимает
вольную роту, когда война подходит к её обозам, выкупает мир, когда
война дороже мира, и требует войны, когда чужие пошлины разоряют торг.
Престол отвечает поборами. А город, разбогатевший быстрее, чем окрепла
корона, объявляет себя вольным: так появляются **торговые республики**,
где правит выборный дож, а не государь.

**Морская осада.** Портовый город, запертый флотом с моря и осаждённый
войском с суши — и тем же врагом, — остаётся без подвоза вовсе и падает
**вдвое быстрее** обычного. Воеводы это знают: войско идёт туда, где
уже стоит свой флот, а флот — туда, где уже стоит своё войско.

Всё это видно во вкладках «Языки», «Посольства и тайны», «Гильдии» и
«Политика».

### Блок 12 — вещи, места, ремёсла, летописи, законы и великие бедствия

Этот блок сделан ради одного правила: **не мир подстраивается под
подземелье, а подземелье берёт факты из истории**. Всё, что здесь
появляется, к концу прогона лежит на карте готовым — с именем, годом,
хозяином и строкой о том, чем это место памятно.

**Вещи, у которых есть имя.** Меч, венец, книга, чаша: двадцать два рода
вещей, четырнадцать материалов, двадцать свойств и десяток проклятий.
Вещь никогда не висит в пустоте — у неё либо владелец, либо место. Вся
**цепочка рук** хранится целиком: кто держал, в каком году и как получил
(выковал, получил в дар, взял с боя, нашёл в кургане). Вещи теряются в
битвах, уходят в землю вместе с хозяином, всплывают через тысячу лет у
чужого народа — и слава их растёт с каждым таким переходом.

**Места истории.** Курганы героев, руины павших городов, логова
чудовищ, клады, поля решающих битв, затонувшие города, запечатанные
чертоги. У каждого места есть глубина (1–5), сторож, добро, содержимое и
одна строка: «здесь лежит тот, кто убил дракона в таком-то году». Места
живут своей жизнью: их обживают твари, их вскрывают искатели — и не все
возвращаются, — их обваливает время.

**Именные чудовища.** Драконы, змеи, кракены, титаны, праматери пауков —
и ночные твари, которые селятся в городе и берут по одному. У чудовища
есть логово, счёт убитых, клад и тот, кто его в конце концов прикончил.
Из кости убитого куют оружие, и это оружие помнит, из кого оно сделано.

**Ремёсла.** Двадцать восемь ремёсел шести семей, от тяжёлого плуга до
киля с прямым парусом. Открытие делают в одной державе и перенимают у
соседей и торговых партнёров — но не у врагов. Ремёсла дают державе не
очки, а разницу: кто-то строит осадные башни, кто-то ходит в открытое
море, кто-то кормит вдвое больше ртов с той же земли.

**Городские события.** Двадцать пять мелких происшествий: пожар,
наводнение, мор, бунт, обвал, крушение, стена, мост, гавань, маяк,
водовод, книгохранилище, школа, рынок, часы, ярмарка, суд, устав,
стража. Из них летопись и состоит между войнами.

**Летописцы и легенды.** В городе садятся писать погодные записи — и
пишут веками, поколение за поколением, передавая перо от учителя к
ученику. Но летопись — не протокол: у свода есть **пристрастие**
(сухой, придворный, храмовый, желчный, склонный к чудесам) и своя
**точность**. Что не сходится, то искажается по-своему: число войска
растёт, победа и поражение меняются местами, случайность становится
чудом, неудобное замалчивается, две беды путаются в одну, вина
перекладывается на соседа. Своды теряются и находятся снова.
А **легенды** живут отдельно от событий: рождаются лет через полтораста
после того, как всё случилось, и меняются в пересказе — рядом с каждой
записано, как было на самом деле.

**Законы и реформы.** Пятнадцать реформ: денежная, единые меры, перепись,
почта, свод законов, правильный суд, городские уставы, запрет частных
войн, освобождение рабов, веротерпимость, равенство народов, постоянное
войско, набор, школы, государев совет. Реформа меняет державу (рост,
войско, торговлю, порядок, учёность) и ссорит с знатью. Удачную
перенимают соседи — и если её завели шестеро, она становится **общим
порядком**, а державу, которая была первой, поминают в каждом своде.

**Великие бедствия.** Четыре беды, после которых меняется сама карта:

* **Погружение** — земля уходит под воду за одну ночь. Города ложатся на
  дно затонувшими местами, уцелевшие уплывают и ставят город на чужом
  берегу: держава, потерявшая родину, продолжается в одном поселении, и
  потомки её ещё тысячу лет зовут себя по земле, которой нет на карте.
* **Раскол суши** — там, где был перешеек, встаёт пролив. Сухопутные
  связи рвутся и становятся морскими, караваны ищут обход веками, а
  деревни по обе стороны разлома через сто лет говорят уже по-разному.
* **Долгая Тьма** — единственная беда без списка земель: она приходит ко
  всем разом, и спрятаться от неё негде.
* **Пробуждение в глубине** — оно не случается само. Его поднимает след
  давнего нашествия, пролежавший под землёй семь веков и больше: это не
  новое зло, а остаток старого. Великий чертог пустеет за сезон и
  остаётся запечатанным.

Великие беды делят одну память на всех: мир, который только что тонул,
не раскалывается назавтра. И у бед есть предел — чем ближе мир ко дну,
тем слабее их рука: всемирная беда должна быть страшной, но не
последней.

**Народ может уйти из мира совсем.** Такое случается редко, и летопись
различает, убила народ беда, подкосила или он угас сам. Назад такие не
возвращаются. Всё это собрано в разделе «Как менялся мир»: великие беды,
затопленные и разрезанные земли, места, куда не ходят, и народы,
которых больше нет.

Всё это видно во вкладках «Вещи», «Места», «Чудовища», «Ремёсла»,
«Своды и легенды», «Законы» и «Как менялся мир».

### Блок 13 — цепи бедствий: беда, которая помнит прежнюю беду

Мир, где бедствия случаются подряд и порознь, читается как погода. Здесь
у части бед есть **родословная**, и её видно в летописи. Две нити лежат
в природе вещей:

* **След.** Уцелевший военачальник, кладка, печать, прореха. Он спит
  веками и однажды просыпается сам.
* **Глубина.** Осколок великого нашествия, пролежавший под землёй семь
  веков и больше, поднимается и пустошит чертог.

Третья нить человеческая: кто-то живой открывает дорогу назад. Способов
**двенадцать**, и мир берёт тот, для которого у него нашлись
действующие лица:

| Путь | Кто открывает дверь |
|---|---|
| государь | едет за силой на древнее поле |
...
| наследство | проклятая вещь переходит к тому, кто её не отдаст |
| гильдия | ведёт штольню глубже, чем вели деды |
| война | войско роет укрепления там, где рыть не следовало |
| голод | гонит селиться туда, куда в сытые годы не ходили |
| легенда | пересказана так долго, что стала указанием |
| кровь рода | в старом доме рождается тот, в ком прошлое проступает |
| пленник | взятый в ту войну, оказывается не зверем |
| срок вышел | не делает никто — просто пришло время |
...
и стандартный тест ругается, если мир обошёлся одним-двумя.

Чтобы это работало, **места теперь помнят, какая беда их оставила**:
поле битвы знает своё нашествие, руины — беду, сжёгшую город. По этой
памяти государь и находит дорогу.

Вот как это читается в летописи:

```
Цепь от 1063 года
  Нашествие Бледного Полчища — нашествие нежити, 1063 год, погибло 16 876
        └─ через 590 лет:
      Нашествие Тихого Воинства — нашествие нежити, 1653 год
          выросло из следа: Гробница Бледного Полчища
            └─ через 1354 года:
          Нашествие Костяного Полчища — нашествие нежити, 3007 год
              выросло из следа: Уцелевший военачальник Умбраис Холодная
        └─ через 1775 лет:
      Рой Чёрного Гнезда — рой адских насекомых, 2838 год
          призвано государем державы по имени Галярыэль
          наследие беды «Нашествие Бледного Полчища»
```

**Связанной оказывается не всякая беда** — обычно от шести до семнадцати
процентов. Цепь должна быть редкой, иначе она перестаёт быть цепью и
становится правилом. У выездов за силой своя память: мир, где государь
только что пал во зло, не даёт второго такого же назавтра.

Всё это видно во вкладке «Цепи бедствий».

### Блок 14 — хозяйство: чем держава живёт

Торговля в генераторе была с седьмого блока: земли давали товар, державы
считали нехватку и возили недостающее караванами и кораблями. Не хватало
одного — **ясного ответа на вопрос, за счёт чего эта держава живёт**.

Теперь на достаток работают четыре вещи разом:

* **Земля.** Что родит почва, что лежит в недрах, насколько земля
  плодородна, богата рудой и магией — всё берётся с карты.
* **Руки народа.** Промысел расы работает где угодно: дворф и на равнине
  останется рудознатцем и найдёт болотное железо там, где человек
  пройдёт мимо; эльф и в степи разведёт сад. Дворфы дают металл, камень
  и самоцветы, высшие эльфы — чародейные снадобья, кошколюды —
  пряности, панцирный народ — соль и рыбу. Числа здесь вдвое-втрое
  меньше, чем у местности: с голой пустыни дворф хлеба не снимет, но
  там, где земля и промысел сходятся, и получается то, чем державу
  знают у соседей.
* **Промыслы народа** — те, что вписаны не расе, а конкретному народу:
  солевары, виноградари, ныряльщики, травники.
* **Ремёсла.** Двадцать одно из двадцати восьми ремёсел блока 12 теперь
  что-то даёт: отводные каналы — хлеб, доменная печь — металл,
  прозрачное стекло — самоцветы и снадобья, океанский корабль —
  пряности. До этой связки две державы одной расы на одной земле были
  неразличимы; теперь одна живёт мехами на шесть десятых, а соседка той
  же расы — самоцветами.

Итог сводится в одну строку, которую видно и в летописи, и в срезе теста:

```
хозяйство: живёт пряностями (33%), чародейными снадобьями (21%)
           и хлебом (12%); достаток: в достатке
не хватает: металл (62%), камень (31%)
ремёсел освоено: 11 из 28
```

Опора считается не по тому, чего больше всего снимают, а по тому, что
**приносит**: горсть самоцветов стоит обоза зерна, и караванные пути
стоят именно на этом. Достаток же меряется не абсолютом, а серединой по
миру: мир, где все освоили доменную печь, богаче первобытного в разы, и
«богатой» держава бывает только среди соседей.

### Политическая карта: какая раса какие гексы желает

Границы держав на карте рисуются волной от каждого живого города:
гекс отходит ближайшему, а через него — его державе. Волна идёт не по
числу шагов, а по цене пути — через болото вдвое хуже, через хребет
почти никак, вдоль реки далеко.

К цене пути добавлено **желание**: насколько эта земля своя для этого
народа. Дворф лезет в горы и в ближние долины и не идёт в болото; эльф
держится леса; человек берёт всё, где родится хлеб; панцирный народ —
побережье и отмели. Цена шага делится на желание, и чужая земля
обходится державе втрое-вчетверо дороже.

Без этого граница ложилась лентой: волна шла по самой дешёвой дороге —
вдоль рек и по равнинам, — и держава дворфов вытягивалась через степь
на пол-материка просто потому, что по степи идти легче, чем по своим же
горам. Теперь между державами остаётся **серая зона**: земли, до
которых никому не хочется тянуться. Нежеланные гексы прирастают позже,
когда держава вырастет настолько, чтобы дотянуться и до них.

На пробной карте это видно по цифрам: доля занятой земли, лежащей в
желанной для народа местности, поднялась с 53–64% до 68–83%, а к концу
истории снова сползает к семидесяти — большие державы всё-таки
дотягиваются до чужого.

### Блок 15 — причинность: из чего вырастают события

До этого блока мир был большим набором связанных сущностей, но события в
нём случались сами по себе. Теперь у истории есть ткань.

**След.** Крупное событие оставляет факт, с которым живут державы, земли
и народы: обида, притязание на отнятый город, утрата, позор и слава,
клятва, иго, зависимость от чужого подвоза, рубец земли от бедствия,
голодная память. У следа есть сила и скорость забвения: голодная память
стирается за полтора века, притязание на землю живёт девять столетий,
рубец бедствия — тысячу лет. Пока след жив, его читают поводы к войне,
выбор жертвы и отношения держав.

**Зерно.** Кроме следа событие может оставить отложенное последствие с
годом всхода: месть через полтораста лет, возврат города через восемь
поколений, бунт в городе, который когда-то взяли силой, сброс чужого
старшинства, голод в державе, привыкшей к чужому хлебу. Созрело зерно —
подсистема пробует его прорастить обычным порядком: война объявляется
войной, бунт поднимается бунтом. Новых способов ломать мир здесь нет.

**Несбывшееся.** Всходит далеко не всё. Зерно может не дождаться своего
года, а может застать мир, в котором мстить уже некому. Часть такого
попадает в летопись отдельной короткой записью — «Месть, которой не
будет», «Земля, которую не вернули», «Иго, которое стерпели». Мир, где
сбывается всё задуманное, выглядит ненастоящим.

Отсюда же берётся **историческая инерция**: подписанный мир не
превращает вражду в дружбу. Отношения держав тянут к себе старые счёты
и старое добро, и вражда остывает ровно так же медленно, как выветривается
след.

```
  Нить из 7 звеньев, 1428 год:
    995: Война: Война за обиду
    |
      1000: Мир, подписанный в городе Кхаз-азгарум
      |
        1171: Война: Война отместки
        |
          1178: Договор в городе Разтехар
          |
            1378: Война: Третья война за Дальний Берег
```

### Блок 16 — память людей и связи между ними

Держава помнит войны, а человек помнит свою жизнь. Государь, у которого
на чужой войне погиб отец, и государь, у которого с той же державой
связаны только выгодные обозы, — это два разных государя.

Что запоминается: гибель родича, плен, предательство союзника, победа,
поражение, гибель родного города, обойдённый престол, брак, возвышение
чужой рукой, боевое товарищество. Память слабеет, но у разных людей
по-разному: **мстительный помнит обиду до могилы, не помнящий обид
прощает за десять лет** — за это отвечают черты нрава из блока 8.
Изредка воспоминание переиначивается, и человек помнит уже не то, что
было.

**Связи** живут своей жизнью: дружба, вражда, соперничество, уважение,
любовь, долг, наставничество, предательство. Политика ссорит друзей,
годы превращают вражду в холодное уважение, обойдённый престолом
становится соперником, а потом и врагом — и это видно в летописи как
цепочка: `соперничество → вражда`.

### Блок 17 — три причины войны

У значимых событий причина не одна:

* **гласно** — что объявлено народу;
* **расчёт** — чего на самом деле хотят в совете;
* **лично** — что помнит сам государь.

Совпадать они не обязаны:

```
Гласно война идёт за одно: защита истинной веры.
На деле в совете считают другое: рудники за рекой.
А государь помнит своё: годы, проведённые в чужом плену.
```

Личная причина — редкость: она появляется только там, где у государя
действительно есть что вспомнить об этом соседе. Зато она же поднимает
войны, которых по расчёту не было бы вовсе.

### Блок 18 — демография и переселения народов

Мир и раньше считал население — по племенам, городам и державам. Не
хватало взгляда сверху: какая земля переполнена, какая пуста и куда
пойдут люди, если их сгонит с места беда.

**Теснота** считается не абсолютом, а долей: сколько душ приходится на
землю по сравнению с тем, сколько эта земля тянет (плодородие, пригодность
для жизни, ёмкость). Одна и та же мера работает и в мире на тридцать тысяч
душ, и в мире на семь миллионов.

**Гонит с места** шесть причин: теснота, голод, война, мор, холода,
разорённая земля и чужой закон. У каждой свой вес: пустой амбар гонит
вернее тесноты, а указ о единой речи — вернее голода.

**Идут не куда попало.** Земля должна быть желанной этому народу (те же
предпочтения, по которым рисуются границы), не переполненной и не слишком
опасной. Мореходы уходят и за воду, остальным дорога кончается берегом.

**Принимают не всех.** Пустая земля не отказывает никому. Чужая держава
решает сама: единокровцев примут охотнее, злой народ не примут вовсе,
голодная держава не примет никого. Непринятые теряют в пути до половины
и оседают, где придётся, а отказ остаётся обидой — с обеих сторон.

```
  Самые большие уходы
     1910  Люди         Молчаливая Долина -> Дальние Холмы, 44180 жизней, война
     2540  Тёмные эльфы Аноэль -> Багровые Ковыли, 40822 жизни, теснота
           Роэнанфорт: отказали
     2560  Тёмные эльфы Аноэль -> Багровые Ковыли, 20040 жизней, теснота
           Роэнанфорт: приняли
```

Принятые меняют расовый состав державы — и однажды вспомнят, чья это
была земля.

### Блок 19 — растворение народов

Завоевание кончается не миром. Через три поколения внуки побеждённых
говорят на языке победителей — или наоборот, если победителей горстка,
а побеждённых земля.

Раз в десятилетие каждая многонародная держава взвешивает две силы:

* **тягу двора** — долю титульного народа, письменность его языка,
  освоенные ремёсла, закон о равных правах;
* **упор меньшинства** — его долю, накопленную обиду, иную кровь, иную
  веру и старые счёты с короной.

Из этого выходит один из четырёх исходов:

| Исход | Что происходит |
|---|---|
| **Растворение** | город меньшинства перенимает речь и обычаи двора; язык уходит первым, имена держатся дольше |
| **Двор заговорил по-здешнему** | своих у короны меньше пятой части — и грамоты начинают писать на языке покорённых |
| **Смешанный народ** | из двух народов выходит третий, которого своим не считают ни те ни эти |
| **Запрет речи** | жестокая держава объявляет чужой язык речью мятежников: это ускоряет растворение и оставляет обиду, которая через век поднимает бунт |

Причину растворения летопись называет: торг, двор, вера, брак, закон —
или просто время, когда никто ничего не запрещал.

### Блок 20 — смуты: держава против самой себя

Прежде междоусобица решалась одним броском: дом поднимался, и тут же
было известно, чем всё кончилось. Теперь у смуты есть всё то же, что и у
войны с соседом, только обе стороны свои.

**Восемь причин:** спор наследников, раскол веры, борьба знати,
сепаратизм окраин, голодный бунт, мятеж неоплаченного войска, ночной
переворот, города против короны.

**Раздел.** Держава делится не случайно: за мятежом идут города вокруг
родового гнезда вожака и те, где молятся не так, как в столице; за
короной — столица и ближние земли.

**Годы.** Каждый год — стычка под чьими-нибудь стенами, переход города
на другую сторону или просто убыль людей. Затянувшуюся смуту чаще
кончают переговорами, чем победой.

**Четыре исхода:** корона удержалась, престол взят мятежником, держава
раскололась надвое, стороны сели за стол. Пока смута идёт, держава не
начинает войн с соседями — ей не до них.

### Блок 21 — заговоры: обида, которая растёт годами

Тайное дело больше не случается по броску кости. У него шесть ступеней:

```
обида → сговор → клятва → ожидание → удар → награда или плаха
```

Заговор заводится там, где есть кому обижаться: обойдённый престолом
родич, дом, у которого корона отняла вольности, проповедник старого
обряда, городской старшина. Он растёт участниками — и с каждым новым
человеком тайна тоньше. Бьёт не когда захочет, а когда государь слаб:
смута, война, голод, старость, малолетний наследник при регенте.

Вожак умер — дело переходит к тому, кто ещё жив. Государь умер сам —
дело чаще всего переписывают на нового. Раскрытое кончается плахой, и
родня казнённых это помнит; удавшийся удар сажает заговорщика на
престол или срывает державу в смуту.

### Блок 22 — настройки: все числа движка шкалами

Шестьдесят восемь чисел — от скорости размножения народов до частоты
бедствий, смут и заговоров — вынесены на шкалы 0…100 в одиннадцати
разделах. Пятьдесят — «как задумано»; края отличаются от середины в
два-четыре раза, и шаг у шкалы кратный, а не линейный.

Каждую шкалу можно **зафиксировать замком**: тогда кнопка «Бросить
кости» её не тронет. Отсюда и берутся миры по заказу: «войны редки,
бедствия часты, остальное — как выпадет».

Положения шкал хранятся в самом мире, поэтому летопись по сиду
воспроизводится вместе с ними. Из консоли то же самое:

```
python main.py --cli --tune war_rate=15,calamity_rate=85
python main.py --tune-list
```

### Блок 23 — Worldforge: картогенератор внутри программы

Карту больше не нужно брать из посторонней программы: картогенератор
**TECTONIC WORLDFORGE v5.2** перенесён в «Хронист» целиком, со всеми
слоями и надстройками. Перенос точный — при одном сиде и одних
ползунках карта выходит та же, гекс в гекс: это проверено сверкой с
исходным генератором по всем 22 слоям и по всему JSON-хвосту, от высот
и биомов до недр, логов и племён.

Что считается:

* **тектоника** — ядра материков, искажённая мозаика плит, напряжение
  на стыках: сходятся — растут горные пояса, расходятся — впадины;
* **рельеф** — континентальный шельф, горячие точки, острова,
  одинокие стратовулканы 2,5–5 км прямо из равнины;
* **эрозия** — капли воды точат склоны и сносят наносы в низины;
* **океан** — уровень моря перцентилем высот, течения тёплые к полюсам
  и холодные к равнику;
* **климат** — широта и высота, материковость, зональные пояса дождей
  и сухих тропиков, лето и зима порознь;
* **вода** — приоритетное заполнение впадин под озёра, сток, порядок
  Штралера до седьмого, дельты с рукавами в устьях больших рек;
* **биомы** — сорок видов с поправкой на испарение: болота по дренажу,
  солончаки в бессточных впадинах, мангры, дюнные эрги и каменистые
  хамады, оазисы, зелёные ленты рек по пустыне, верещатники у моря;
* **земли** — океаны, моря, заливы, материки, острова, архипелаги,
  хребты и реки со своими именами; именные вершины с высотой;
* **недра** — металлогенические пояса на каждое семейство руд, семь
  ступеней запасов, сортность, доступность, примеси; алмазы в
  кратонах, изумруды в орогенах, порфировые пояса магмы;
* **живность** — звери, скот, крупная дичь и магические существа по
  биомам, семнадцать видов добра земли, морские промыслы;
* **прочее** — плодородие, грунтовые воды и водоносные слои, водопады,
  типы берегов, узлы магии, дикость округи, 16 местных бедствий и
  примет, 13 родов древних логов, стоянки племён и лента климата на
  всю историю мира.

Настройки: размер (малая 160×96, средняя 280×168, большая 500×300),
отдельный сид карты, от одного до восьми материков, замкнутость мира по
долготе и 18 ползунков — у каждого свой замок, как у шкал мира.

Карту видно прямо в мастере, её можно перебрасывать, пока не
понравится, и сохранять файлом `.world` — формат тот же, что у
исходного картогенератора.

```
python main.py --cli --map-random средняя --save-map карта.world
python main.py --map-knob-list
```
...
* **хозяйство** — сколько у держав разных опор, чем живут чаще всего,
  кто богаче и беднее всех и чем. Здесь же ловится мир, где все державы
  живут одним и тем же;
* **цепи бедствий** — сколько бед выросло из прежних, какой самый долгий
  разрыв между родителем и наследником, сколько было выездов за силой и
  павших государей. Здесь же ловятся два перекоса: мир, беды которого
  совсем не помнят друг друга, и мир, где из прежней беды выросла почти
  каждая;
* **худший век мира** — по переписи мира, а не по одному бедствию.
  Раз в десять лет генератор записывает, сколько в мире душ, племён,
  городов, стран, идущих войн и сколько на мире мрака. По этой переписи
  тест находит худшие десять лет и худший век, называет виновника — те
  беды, что шли в мире в те годы, — и показывает лучший век и самый
  пустой. Здесь же ловятся два перекоса: мир, кончающий историю в
  пятидесятой доле от лучшего века, и мир, проведший во всемирной тьме
  больше половины своей истории.
...
Генератор обязан делать **разные** миры, а не один мир с разными
именами. Отдельная команда прогоняет несколько миров подряд и сравнивает
их между собой:

```
python tools/variety.py --worlds 10 --years 4000
```

Она отвечает на четыре вопроса:

* **Не повторяются ли имена.** Сколько придуманных имён — земель,
  народов, держав, богов, вещей, личностей — встречается больше чем в
  одном мире.
* **Не повторяются ли фразы.** Тексты всех миров сводятся в одну кучу,
  имена в них заменяются заглушками, и считается, какая доля фраз
  уникальна и какие шаблоны выскакивают чаще прочих. Именно так были
  найдены и расширены самые заезженные строки летописи.
* **Разные ли миры по устройству.** Население, страны, войны, веры,
  языки, бедствия, вымершие народы — разброс от беднейшего мира к
  богатейшему по каждому показателю.
* **Нет ли зацикленности.** История делится на три части, и считается,
  насколько набор событий последней трети повторяет первую. Мир, у
  которого третья тысяча лет неотличима от первой, читать неинтересно.
...
worldgen/
    ...
    tongues.py          языки народов: звуковые законы, письменность, родство
    embassy.py          наказы послам, дары и то, как отвечает чужой двор
    espionage.py        тайные дела: подкуп, яд, заговор, соглядатаи
    guilds.py           гильдии: касса, вольности и вольные города
    narrative_tongues.py тексты о языках и письменности
    narrative_embassy.py тексты о посольствах и переговорах
    narrative_union.py  тексты о династических униях
    narrative_spies.py  тексты о тайных делах
    narrative_guilds.py тексты о гильдиях и вольных городах
    ...
    systems/            подсистемы: география, народы, основания, эпохи,
                        ...
                        войны, политика, крепости и роты, языки, посольства,
                        унии, тайные дела, гильдии
