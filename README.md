...
1. Вкладка **Actions** этого репозитория → последний зелёный запуск
   «Хронист для Windows».
2. Внизу, в разделе **Artifacts**, скачать `Хронист-Windows`.
3. Распаковать и запустить `Хронист.exe` двойным щелчком.
...
каждом пуше; по метке версии (`git tag v1.0`) тот же файл попадает в
Releases.
